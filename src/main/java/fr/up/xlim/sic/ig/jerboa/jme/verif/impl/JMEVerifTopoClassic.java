...
public final class JMEVerifTopoClassic {
    ...
    JMERuleError verifDimension(JMERule rule) {
        ...
        error = verifDimensionGraphArcs(rule, modDim, left);
        if (error != null) {
            return error;
        }
        ...
        error = verifDimensionGraphArcs(rule, modDim, right);
        if (error != null) {
            return error;
        }
    ...
    JMERuleError verifDimensionGraphNodes(JMERule rule, int modDim, JMEGraph graph) {
        ...
        /*@ loop_invariant
          ...
          @           && ((JMENode)(graph.nodes).get(a)).orbit.dim[b] <= modDim));
          @ assignable \nothing;
        ...
        for (int j = 0; j < leftNodesSize; j++) {
            ...
            /*@ loop_invariant
              ...
              @           && ((JMENode)(graph.nodes).get(j)).orbit.dim[c] <= modDim);
              @ assignable \nothing;
              @ maintaining \invariant_for(graph) && \invariant_for(rule) && graph != null && rule != null
    ...
    /*@ nullable */ JMERuleError verifDimensionGraphArcs(JMERule rule, int modDim, JMEGraph graph) {
        ...
        /*@ loop_invariant
          ...
          @ assignable \nothing;
          @ maintaining \invariant_for(graph) && \invariant_for(rule) && graph != null && rule != null
          @  && (rule.left == graph || rule.right == graph) && leftArcs == graph.arcs;
