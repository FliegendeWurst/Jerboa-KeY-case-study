...
public class RulePostProcessTab extends JPanelTabDescription implements JMEElementView {

	private static final long serialVersionUID = 4874899233943603128L;
	private ExpressionPanel textPostprocess;
	private JButton bntApply;
	private JButton btnReset;
	private JMERule rule;
	private RuleView owner;

	public RulePostProcessTab(RuleView view) {
		super(view.getTabbedPane(), "PostProcess");
		this.owner = view;
		...
		JLabel lblComment = new JLabel("Postprocess:");
		...
		textPostprocess = new ExpressionPanel("postprocess", rule.getModeler(),
				new JerboaLanguageGlue(rule, LanguageState.POSTPROCESS));
		textPostprocess.addFocusListener(new FocusAdapter() {
			@Override
			public void focusLost(FocusEvent e) {
				save();
			}
		});
		textPostprocess.addKeyListener(new KeyListener() {
			@Override
		...
		panel_5.add(textPostprocess, BorderLayout.CENTER);
		// scrollPane.setViewportView(textPostprocess);
	...
	@Override
	public void reload() {
		if(!rule.getPostProcess().equals(textPostprocess.getText()))
			textPostprocess.setText(rule.getPostProcess());
	}

	public void save() {
		rule.setPostProcess(textPostprocess.getText());
		rule.update();
		owner.check();
	}
