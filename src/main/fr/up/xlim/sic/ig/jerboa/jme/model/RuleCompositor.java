...
public class RuleCompositor {
	...
	public final JMERuleAtomic getIteration(int iter)  {
		...
		for(int it=0;it<iter;it++) {
			...
			System.err.println("cr�ation des noeuds : ");
			...
			System.err.println("cr�ation des copies : ");
			...
			// renommage des arcs préexistants
...
//				arcs.addAll(r.getArcs());
						...
						for(JMEArc a : mapNodeToItsArcs.get(copieLine.get(i))) {
							if( a.link(copieLine.get(i),copieLine.get(j)) ) {
								...
								if(newO>-1) {
									...
									r.removeArc(a);	 // si on es tpas sur le motif de base on ne crée pas les noeuds non filtré !
								}
							}
						}
					}
			...
			System.err.println("d�placement des noeuds : ");
			int spacement = 150;
			for(int k=0;k<rightSize2;k++) {
				for(int i=0;i<rightSize1;i++) {
