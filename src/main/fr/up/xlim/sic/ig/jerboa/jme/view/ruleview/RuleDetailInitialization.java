...
public class RuleDetailInitialization extends JPanelTabDescription implements JMEElementView {
	...
	public RuleDetailInitialization(RuleView view) {
		super(view.getTabbedPane(), "Initialization");
		this.owner = view;
		this.rule = owner.getRule();
		this.rule.addView(this);

		setLayout(new BorderLayout(0, 0));
		label = new JLabel(getInitialTitle());
		...
		textHeader = new ExpressionPanel("header of " + rule.getFullName(), view.getModeler(), null);
	...
	public void save() {
		// rule.setInit(textHeader.getText());
		// rule.update();
		rule.check();
		throw new Error(
				"Pas implemented, dans le cas ou on devait separer l'entete normalement Val a dit que son langage pourrait s'en occuper");
