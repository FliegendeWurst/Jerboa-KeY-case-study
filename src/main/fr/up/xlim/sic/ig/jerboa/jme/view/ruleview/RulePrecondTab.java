...
public class RulePrecondTab extends JPanelTabDescription implements JMEElementView {
	...
	public RulePrecondTab(RuleView view) {
		super(view.getTabbedPane(), "Precondition");
		this.owner = view;
		...
		JLabel lblComment = new JLabel("Precondition:");
		...
		textPrecond = new ExpressionPanel("precondition", rule.getModeler(),
				new JerboaLanguageGlue(rule, LanguageState.PRECONDITION));
	...
	@Override
	public void reload() {
		if(!rule.getPrecondition().equals(textPrecond.getText()))
			textPrecond.setText(rule.getPrecondition());
	}

	public void save() {
		rule.setPrecondition(textPrecond.getText());
		owner.check();
	}
